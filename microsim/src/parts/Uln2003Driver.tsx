...
export function Uln2003DriverPart({ part, selected, pinStates, onPinClick }: Uln2003DriverPartProps) {
  ...
  return (
    <g transform={`translate(${part.x}, ${part.y}) rotate(${part.rotation ?? 0})`}>
      <defs>
        {/* Mask that cuts physical holes through the green PCB layer */}
        <mask id={maskId}>
          {/* White base = keep PCB filled */}
          <rect
            x={-halfW}
            y={-halfH}
            width={halfW * 2}
            ...
            fill="#ffffff"
          />
          {/* Black circles = subtract holes from PCB */}
      ...
      {/* Green PCB Board (Masked for true hole cutouts) */}
      <rect
        x={-halfW}
        y={-halfH}
        width={halfW * 2}
      ...
      {/* Metallic Annular Copper Pads around mounting hole cutouts */}
      {cornerHoles.map((hole, i) => (
        <circle
          key={`pad-${i}`}
          cx={hole.x}
