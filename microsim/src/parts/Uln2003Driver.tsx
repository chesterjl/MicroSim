...
export function Uln2003DriverPart({
  part,
  selected,
  pinStates,
  onPinClick,
  ...
  const halfW = (def.widthUnits * GRID) / 2; // 110
  const halfH = (def.heightUnits * GRID) / 2; // 80
  ...
  // Dynamic connector housing width based on top motor pins
  ...
  return (
    <g transform={`translate(${part.x}, ${part.y}) rotate(${part.rotation ?? 0})`}>
      ...
      {/* Top Stepper Motor Connector Block (White Header) */}
      <rect
        x={minMotorX - 10}
        y={-halfH - 2}
      ...
      {/* Top Motor Pin Silkscreen Labels */}
      {motorPins.map((pin) => (
        <text
          key={`label-${pin.id}`}
          x={pin.x * GRID}
        ...
        >
          {pin.label || pin.id.toUpperCase()}
        </text>
      ))}

      {/* Display the light for each pins that is active */}
      {motorPins.map((pin) => {
        const lit = pinStates?.[pin.id] === "high";
        return (
          <g key={`led-group-${pin.id}`}>
            <circle
              cx={pin.x * GRID}
              cy={pin.y * GRID + 14}
              r={3.5}
              fill={lit ? "#22c55e" : "#14532d"}
              stroke={lit ? "#86efac" : "#052e16"}
              strokeWidth={1}
            >
            </circle>
          </g>
      ...
      {/* On/Off Jumper Header & Shunt */}
      <g transform={`translate(${halfW - 22}, ${-halfH + 20})`}>
        <rect x={-11} y={14} width={12} height={12} fill="#1e293b" rx={1} />
        <rect x={-10} y={16} width={10} height={8} fill="#eab308" rx={1} stroke="#ca8a04" strokeWidth={0.8} />
        <text x={-5} y={32} textAnchor="middle" fontSize={4.5} fontWeight={700} fill="#ffffff" fontFamily="monospace">
          ON/OFF
        </text>
      </g>

      {/* Bottom IN1-IN7 Male Header Block */}
      <rect x={-halfW + 12} y={halfH - 16} width={halfW * 2 - 24} height={10} rx={1.5} fill="#0f172a" />

      {/* Bottom IN Pins Silkscreen Labels */}
      {inPins.map((pin) => (
        <text
          key={`label-${pin.id}`}
          x={pin.x * GRID}
    ...
    <text
      ...
      {/* Interactive Pin Connection Dots */}
      {def.pins.map((pin) => (
        <PinDot
          key={pin.id}
          x={pin.x * GRID}
