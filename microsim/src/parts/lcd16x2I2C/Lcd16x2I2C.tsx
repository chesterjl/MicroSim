...
export function Lcd16x2I2CPart({ part, selected, pinStates, netlist, onPinClick }: Lcd16x2I2CPartProps) {
  ...
  // Wide, prominent outer bezel (Tinkercad style)
  const outerBezelX = -halfW + 20;
  const outerBezelY = -halfH + 18;
  const outerBezelW = halfW * 2 - 36;
  const outerBezelH = halfH * 2 - 28;

  // Blue LCD screen inside bezel
  ...
  return (
    <g transform={`translate(${part.x}, ${part.y}) rotate(${part.rotation ?? 0})`}>
      <defs>
        {/* Mask to cut actual see-through holes in PCB */}
        <mask id={maskId}>
          <rect x={-halfW - 5} y={-halfH - 5} width={halfW * 2 + 10} height={halfH * 2 + 10} fill="#ffffff" />
      ...
      {/* Copper Ring Overlays for Corner Mounting Holes */}
      {cornerHoles.map((hole, idx) => (
